...
{
    public class DetranVerificadorMultasFactory : IDetranVerificadorMultasFactory
    {
        private readonly IServiceProvider _ServiceProvider;
        private readonly IDictionary<string, Type> _Repositories = new Dictionary<string, Type>();

        public DetranVerificadorMultasFactory(IServiceProvider serviceProvider)
        {
        ...
        {
            IDetranVerificadorMultasRepository result = null;

            if (_Repositories.TryGetValue(UF, out Type type))
            {
                result = _ServiceProvider.GetService(type) as IDetranVerificadorMultasRepository;
            }
